...
import com.electro.store.repostories.CartItemRepository;
    ...
    public CartDto addItemToCart(String userId, AddItemToCartRequest request) {
       ...
       items = items.stream().map(item -> {
            if (item.getProduct().getProductId().equals(productId)) {
                ...
                item.setTotelPice(quantity*product.getDiscountedPrice());
                updated.set(true);
            }
            return item;
...
//        cart.setItems(updatedItems);
        ...
        if(!updated.get()) {
            CartItem cartItem = CartItem.builder()
                    .quantity(quantity)
                    .totelPice(quantity * product.getDiscountedPrice())
                    .cart(cart)
                    .product(product)
                    .build();
